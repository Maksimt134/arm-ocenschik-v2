...
// Find the start of the card inside displayed.map(analog => {
const startIdx = code.indexOf('return (\n                <div\n                  key={analog.id}');
if (startIdx !== -1) {
  // Find the end of the return statement
  const endIdx = code.indexOf(';\n            })}\n          </div>\n\n          {visibleCount < filteredAnalogues.length', startIdx);
  if (endIdx !== -1) {
    ...
    code = code.slice(0, startIdx) + newReturn + code.slice(endIdx);
