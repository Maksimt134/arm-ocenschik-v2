...
// 1. Add new imports
const importTarget = `  ArrowLeft\n} from 'lucide-react';`;
const importReplacement = `  ArrowLeft,\n  FileText,\n  FileImage,\n  Globe\n} from 'lucide-react';\nimport { generateDetailedReportHtml } from '../utils/reportGenerator';`;
content = content.replace(importTarget, importReplacement);

// 2. Add new states & remove old handleExport
const stateTargetRegex = /const \[isModalOpen, setIsModalOpen\] = useState\(false\);\n  const \[isGenerating, setIsGenerating\] = useState\(false\);\n  const \[aiText, setAiText\] = useState\(""\);/;

const exportCodeRegex = /const handleExport = \(\) => \{[\s\S]*?printWindow\.document\.close\(\);\n  \};/;

content = content.replace(stateTargetRegex, `const [isModalOpen, setIsModalOpen] = useState(false);
  const [isGenerating, setIsGenerating] = useState(false);
  const [aiText, setAiText] = useState("");
...
content = content.replace(exportCodeRegex, newExportCode);

// 3. Update the export button to open the modal
const buttonRegex = /<button onClick=\{handleExport\} className="bg-blue-600 hover:bg-blue-500 text-white px-8 py-3.5 rounded-xl font-bold flex items-center gap-3 transition-all active:scale-95 shadow-\[0_0_15px_rgba\(37,99,235,0\.4\)\]">/g;
content = content.replace(buttonRegex, `<button onClick={() => setIsExportModalOpen(true)} className="bg-blue-600 hover:bg-blue-500 text-white px-8 py-3.5 rounded-xl font-bold flex items-center gap-3 transition-all active:scale-95 shadow-[0_0_15px_rgba(37,99,235,0.4)]">`);

// 4. Add Export Modal JSX before the last closing </div> of the component
